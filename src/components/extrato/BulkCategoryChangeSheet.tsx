...
import { Tag, Check, ChevronRight, Loader2, AlertCircle, Plus } from "lucide-react";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
import { InlineCreateCategory } from "./InlineCreateCategory";
...
export function BulkCategoryChangeSheet({
  ...
  const handleCreated = (categoryId: string, subcategoryId: string | null) => {
    setSelectedCategory(categoryId);
    setSelectedSubcategory(subcategoryId);
    setShowCreateModal(false);
    
    // If category has subcategories and one was created, stay on category step
    // Otherwise go to subcategory selection if category has subcategories
    const cat = allCategories.find(c => c.id === categoryId);
    if (cat && cat.subcategories.length > 0 && !subcategoryId) {
      setStep('subcategory');
    }
  };
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="bottom" className="h-[85vh] rounded-t-3xl">
        <div className="relative h-full flex flex-col">
          {/* Main Content - hidden when create modal is open */}
          <div className={cn("flex flex-col h-full", showCreateModal && "invisible")}>
            ...
            <ScrollArea className="flex-1 pr-4">
              {step === 'category' ? (
                <div className="space-y-4">
                  ...
                  <Button
                    variant="outline"
                    className="w-full h-12 gap-2 border-dashed border-primary/50 text-primary hover:bg-primary/5"
          ...
          {/* Inline Create Modal - overlays the main content */}
