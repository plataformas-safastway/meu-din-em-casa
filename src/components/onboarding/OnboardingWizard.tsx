...
export function OnboardingWizard({ onComplete, onSkip, initialData }: OnboardingWizardProps) {
  ...
  // Get selected band for subband selection
  ...
  // Derive hasDependents from household structure
  ...
  const handleIncomeBandSelect = (bandId: string) => {
    ...
    // If band has only one subband or user selects, auto-set midpoint
    if (band && band.subBands.length === 1) {
      setIncomeAnchorValue(band.subBands[0].midpoint);
      handleStepComplete(3); // Skip anchor step
    } else {
  ...
  const handleNonMonthlySelect = async (levelId: string) => {
    ...
    const data: OnboardingData = {
      ...
      hasPets,
      hasDependents,
      nonMonthlyPlanningLevel: levelId,
    };
            ...
            >
              ...
              {isSubmitting && (
                <div className="flex items-center justify-center gap-2 text-muted-foreground">
                  ...
                  <span className="text-sm">Preparando seu orçamento...</span>
                </div>
              )}
            </motion.div>
          )}
