import { Building2, CreditCard, Mail, FileText, CheckCircle, AlertTriangle, Clock, XCircle, ArrowRight, HardDrive, Cloud, Sparkles, MapPin } from "lucide-react";
...
const PROVIDER_ICONS: Record<IntegrationProvider, React.ReactNode> = {
  ...
  LOVABLE_AI: <Sparkles className="w-8 h-8" />,
  OPENSTREETMAP: <MapPin className="w-8 h-8" />,
};
...
const PROVIDER_DESCRIPTIONS: Record<IntegrationProvider, string> = {
  ...
  LOVABLE_AI: "IA generativa para insights, OCR de recibos e projeções financeiras",
  OPENSTREETMAP: "Geocodificação reversa para contexto de localização (gratuito)",
};
...
export function IntegrationsOverviewPage({ onNavigate }: IntegrationsOverviewPageProps) {
  ...
  const providers: IntegrationProvider[] = ['OPEN_FINANCE', 'LOVABLE_AI', 'OPENSTREETMAP', 'GOOGLE_DRIVE', 'ONEDRIVE', 'ACQUIRER', 'RESEND', 'ENOTAS'];
